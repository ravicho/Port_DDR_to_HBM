...
#(parameter
    ...
    output wire [31:0]                   num_times,
    output wire [0:0]                    addRandom,
...
localparam
    ...
    ADDR_SIZE_DATA_0      = 7'h34,
    ADDR_SIZE_CTRL        = 7'h38,
    ADDR_NUM_TIMES_DATA_0 = 7'h3c,
    ADDR_NUM_TIMES_CTRL   = 7'h40,
    ADDR_ADDRANDOM_DATA_0 = 7'h44,
    ADDR_ADDRANDOM_CTRL   = 7'h48,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_num_times = 'b0;
    reg  [0:0]                    int_addRandom = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUM_TIMES_DATA_0: begin
                    rdata <= int_num_times[31:0];
                end
                ADDR_ADDRANDOM_DATA_0: begin
                    rdata <= int_addRandom[0:0];
                end
            endcase
        end
    end
...
assign num_times   = int_num_times;
assign addRandom   = int_addRandom;
// int_event_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        int_num_times[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_TIMES_DATA_0)
            int_num_times[31:0] <= (WDATA[31:0] & wmask) | (int_num_times[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_ADDRANDOM_DATA_0)
            int_addRandom[0:0] <= (WDATA[31:0] & wmask) | (int_addRandom[0:0] & ~wmask);
    end
end
