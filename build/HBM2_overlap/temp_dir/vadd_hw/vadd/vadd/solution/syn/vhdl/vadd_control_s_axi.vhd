...
port (
    ...
    addRandom             :out  STD_LOGIC_VECTOR(0 downto 0);
...
-- 0x38 : reserved
-- 0x3c : Data signal of num_times
--        bit 31~0 - num_times[31:0] (Read/Write)
-- 0x40 : reserved
-- 0x44 : Data signal of addRandom
--        bit 0  - addRandom[0] (Read/Write)
--        others - reserved
-- 0x48 : reserved
...
architecture behave of vadd_control_s_axi is
    ...
    constant ADDR_NUM_TIMES_DATA_0 : INTEGER := 16#3c#;
    constant ADDR_NUM_TIMES_CTRL   : INTEGER := 16#40#;
    constant ADDR_ADDRANDOM_DATA_0 : INTEGER := 16#44#;
    constant ADDR_ADDRANDOM_CTRL   : INTEGER := 16#48#;
    ...
    signal int_num_times       : UNSIGNED(31 downto 0) := (others => '0');
    signal int_addRandom       : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_NUM_TIMES_DATA_0 =>
                        rdata_data <= RESIZE(int_num_times(31 downto 0), 32);
                    when ADDR_ADDRANDOM_DATA_0 =>
                        rdata_data <= RESIZE(int_addRandom(0 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    num_times            <= STD_LOGIC_VECTOR(int_num_times);
    addRandom            <= STD_LOGIC_VECTOR(int_addRandom);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_NUM_TIMES_DATA_0) then
                    int_num_times(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_num_times(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ADDRANDOM_DATA_0) then
                    int_addRandom(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_addRandom(0 downto 0));
                end if;
            end if;
        end if;
    end process;
